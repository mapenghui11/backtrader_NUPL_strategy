...
class SOLStrategy(bt.Strategy):
    def __init__(self):
        ...
        # 定义 rsi 指标
        self.rsi = bt.indicators.RSI_SMA(self.datas[0], period=17)
        # 定义 ma 指标
    ...
    def next(self):
        ...
        # 成交量策略：做多
        vol_long = (self.close[0] < self.open[0] and self.vol[0] > 650000) and \
                   (self.rsi[0] < 60) and (self.hist[0] < -0.02) and \
                   (self.close[0] < self.ma20[0]) and self.vol[-1] < (self.vol[0] * 0.5)
        # 成交量策略：做空
        vol_short = (self.close[0] > self.open[0]) and (350000 < self.vol[0] < 550000) and \
                    (self.low[0] > self.ma20[0]) and (65 < self.rsi[0] < 80) and \
                    (self.vol[-1] < (self.vol[0] * 0.6)) and (self.hist[0] > 0.01)
        ...
        if not self.position:
            ...
            if vol_short:
                self.sell()
                short_order_close = self.close[0]
            ...
            if self.position.size > 0:
                ...
                close_vol_long = (self.ma2[0] < self.ma2[-1]) and (self.ma2[-1] > self.ma2[-2]) and \
                                 (self.ma2[-2] > self.ma2[-3]) and (ma2_delta < -0.05)
                close_vol_long_1 = close_vol_long and time_diff.total_seconds() > 3 * 5 * 60
                close_vol_long_2 = (self.high[0] > self.upper[0]) and self.rsi[0] > 65

                stop_vol_long = self.close[0] < self.buyprice * 0.98
                ...
                close_vol_short = (self.ma2[0] > self.ma2[-1]) and (self.ma2[-1] < self.ma2[-2]) and (self.ma2[-2] < self.ma2[-3]) and \
                                  (ma2_delta > 0.045) and (last_ma2_delta < -0.015) and (second_ma2_delta < -0.005)
                close_vol_short_1 = close_vol_short and time_diff.total_seconds() > 3 * 5 * 60
                close_vol_short_2 = (self.close[0] < self.lower[0]) and self.rsi[0] < 30

                stop_vol_short = self.close[0] > self.sellprice * 1.06
                if close_vol_short_1 or close_vol_short_2 or stop_vol_short:
                    self.buy()
...
if __name__ == '__main__':
    ...
    data_SOLUSDT = bt.feeds.PandasData(
        dataname=dataframe,
        fromdate=dt.datetime(2023, 1, 14),
        todate=dt.datetime(2023, 3, 18),
